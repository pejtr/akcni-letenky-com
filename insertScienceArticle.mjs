...
const articleData = {
  ...
  category: "tips",
  author: "Cestovatelský Tým Akční Letenky",
...
async function main() {
  ...
  try {
    ...
    if (Array.isArray(existing) && existing.length > 0) {
      console.log(`Article with slug '${articleData.slug}' exists. Updating...`);
      await connection.execute(
        `UPDATE articles SET title = ?, excerpt = ?, content = ?, featuredImage = ?, keywords = ?, category = ?, author = ?, updatedAt = NOW() WHERE slug = ?`,
        [
          articleData.title,
          articleData.excerpt,
          articleData.content,
      ...
      console.log(`Inserting new article '${articleData.title}'...`);
      await connection.execute(
        `INSERT INTO articles (title, slug, excerpt, content, featuredImage, keywords, category, author, publishedAt, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW(), NOW())`,
        [
          articleData.title,
          articleData.slug,
          articleData.excerpt,
